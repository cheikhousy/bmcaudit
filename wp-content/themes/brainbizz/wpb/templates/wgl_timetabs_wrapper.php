		...
		if ($headings) {
			...
			foreach ($tab_titles_array as $value) {
				echo '<div class="wgl_tab" data-tab-id="'.esc_attr($value['tab_id']).'">';
					echo !empty($value['title']) ? '<div class="tab_title">'.esc_html($value['title'],'brainbizz').'</div>' : '';
					echo !empty($value['sub_title']) ? '<div class="tab_subtitle">'.esc_html($value['sub_title'],'brainbizz').'</div>' : '';
				echo '</div>';
			}
		if ($headings) {
			echo '</div>';
