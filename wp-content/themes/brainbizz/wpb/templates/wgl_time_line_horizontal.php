<?php
    ...
    foreach ( $item_data as $item_d ) {
        ...
        ob_start();
            echo "#$time_line_id .time_line-check:before{
                ...
                box-shadow: 0 0 10px 5px rgba(".BrainBizz_Theme_Helper::HexToRGB($item_d['circle_color']).",0.35);
            }";
            echo "#$time_line_id.time_line-item:hover .time_line-check:before{
                box-shadow: 0 0 10px 10px rgba(".BrainBizz_Theme_Helper::HexToRGB($item_d['circle_color']).",0.35);
            }";
        $styles = ob_get_clean();
        BrainBizz_shortcode_css()->enqueue_brainbizz_css($styles);
    ...
    $output .= '<div '.$time_line_module_attr.' class="brainbizz_module_time_line_horizontal'.esc_attr($time_line_wrap_classes).'">';
