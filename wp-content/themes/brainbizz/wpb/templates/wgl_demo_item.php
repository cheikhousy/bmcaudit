<?php
	...
	// render html
	$output .= '<div class="brainbizz_module_demo_item'.esc_attr($di_wrap_classes).'">';
		if (!empty($di_image)) {
			$output .= '<div class="di_image-wrap">';
				...
				$output .= (bool)$add_button ? '<div class="di_button brainbizz_module_button wgl_button wgl_button-xl"><a '.$button_attr.'>'.esc_html($di_button_title).'</a></div>' : '';
				$output .= (bool)$coming_soon ? '<h5 class="di_label">'.esc_html__( 'Coming Soon', 'brainbizz' ).'</h5>' : '';
			$output .= '</div>';
		}
